/*3,201828000936007,yuzezhong*/
/*in = out = through triangle,they are the same*/
#include <set>
#include <map>
#include <cmath>
#include <cstdio>
...
{
	public:
		//map<vertexId,<inNeighbors id ...>>
		map<int64_t, vector<int64_t>> miv;
		void compute(MessageIterator *pmsgs)
		{
			int64_t val = getVertexId();
			if (getSuperstep() == 0)
			{
				//send vertexId to outNeighbors
			...
			else if (getSuperstep() == 1)
			{
				...
				for (; !pmsgs->done(); pmsgs->next())
				{
					...
					neigh.push_back(msg);
					//send inNeighbors id to outNeighbors
			...
			else if (getSuperstep() == 2)
			{
				val = getVertexId();
				vector<int64_t> &neigh = miv[val];
				vector<int64_t> TwoJumps;

				//put inNeighbors id into TwoJumps
				for (; !pmsgs->done(); pmsgs->next())
				{
					int64_t msg = pmsgs->getValue();
					TwoJumps.push_back(msg);
				}

				OutEdgeIterator it = getOutEdgeIterator();
				for (; !it.done(); it.next())
				{
					for (vector<int64_t>::iterator iter = TwoJumps.begin();iter != TwoJumps.end(); ++iter)
					{
						if (*iter == it.target())
						{
							int addition = 1;
			...
			{
				//conutA is cycle triangle 
				//conutB is in,out,through triangle
				countA = *(int *)getAggrGlobal(0);
				countB = *(int *)getAggrGlobal(1);
				voteToHalt();
				return;
			}
			*mutableValue() = val;
